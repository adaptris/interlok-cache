 ...
 * cacheManager, which can cause problems across the distributed adapters. In these cases the distributed adapters can no longer perform
 * operations on any of the caches related to this cache manager.
 * </p>
 * <p>
 * If you are using ehcache in distributed mode you should consider setting this classes shutdown-cache-manager-on-close to false. Which
 * will not call CacheManager.shutdown(). And instead you can set the jvm property; {@code -Dnet.sf.ehcache.enableShutdownHook=true}
 * </p>
 * .
 */
...
public abstract class EhcacheCache implements Cache {
  ...
  /**
   * Initialises the underlying ehcache CacheManager. If the cache definition already exists (either created in another thread or else
   * created in the ehcache.xml configuration file) then it will be reused. All cache instances are decorated by using a wrapper of
   * {@link TimedExpirationCache} in order to ensure timely eviction of entries.
   *
   */
  @Override
   ...
   * <p>
   * This task is responsible for checking for expired items in the cache and should be configured to run as infrequently as possible whilst
   * still performing evictions in the time required. This will be a relatively heavy weight operation.
   * </p>
   *
   * @param interval
